...
⭐ CORS (Cross-Origin Resource Sharing):CORS is a browser security system that controls whether one website can request data from another website (cross-origin).

⭐ Why CORS?
To protect the user from malicious sites accessing sensitive data (example: bank details).

⭐ What is Origin?
Origin = Protocol + Domain + Port
If any part is different → cross-origin.
Example:
http://localhost:3000 ≠ http://localhost:5000

⭐ How CORS Works
Browser sends request to another origin.
Server checks and replies with permission header.
